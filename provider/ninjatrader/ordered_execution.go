// W117 F2 (9b379c8c, re-derived on today's code) — consume execution evidence
// in TCP receive order. The owning durable consumers (entry fill handling,
// armed order-state changes, close recording) run SYNCHRONOUSLY on the TCP
// read goroutine, before advisory channel fanout, so order/fill/close
// interleaving arrives in the order the wire delivered it.
...
// goroutine, before advisory fanout. Callbacks must not wait for broker
// replies. Database backpressure deliberately delays feed reception rather
// than losing or reordering execution evidence. No router/cache/connection
// lock is held.
