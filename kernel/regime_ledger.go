...
// loss_streak_n) are per-strategy and read at cycle time; their SHIPPED
// defaults are declared here. Env knobs are resolved live.
func LogRegimeBootLedger() {
	...
	logger.Infof("🛡️ regime ledger: loss_streak=%d consecutive (Studio regime.loss_streak_n, default 4; 0=off) · pause=%dmin (env LOSS_STREAK_PAUSE_MIN)", DefaultLossStreakN, LossStreakPauseMin())
