	...
	case "model_management":
		...
		if lang == "zh" {
			return strings.Join([]string{
				...
				"- 推荐 provider：claw402。claw402 是 VL 官方推荐方案，按次付费，使用 Base 链 EVM 钱包 + USDC 支付。",
		...
		return strings.Join([]string{
			...
			"- Recommended provider: claw402. claw402 is the VL recommended pay-per-use option that uses a Base chain wallet + USDC.",
