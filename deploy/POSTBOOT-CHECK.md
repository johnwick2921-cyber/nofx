...
| 8 | first planner read at max | the FIRST `🧠 planner call (reasoning=… wire=… cap=…)` after boot must read `reasoning=max`; the per-trader `🧬 plan lifecycle: … plan_reasoning=max` must too. **Owner-approved exception (A2 update + timing correction, owner 07:3x CT 2026-09-26):** the planner HIGH A/B runs from THIS weekend boot via the owner's `.env` — a `reasoning=high` reading is the owner's test in force, NOT a boot defect; the code default stays max and nothing is hard-coded. Report it as a NOTE, not a FAIL, while the owner's A/B is in force. |
