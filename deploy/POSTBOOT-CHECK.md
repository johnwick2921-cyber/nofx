...
| 8 | first planner read at max | the FIRST `🧠 planner call (reasoning=… wire=… cap=…)` after boot must read `reasoning=max`; the per-trader `🧬 plan lifecycle: … plan_reasoning=max` must too. |
