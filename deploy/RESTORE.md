...
- **Timer:** `~/.config/systemd/user/vl-backup.timer` → fires **05:00 & 17:30 CT**
...
- **Service:** `vl-backup.service` (oneshot) → runs `deploy/vl-db-backup.sh`.
...
- **Location & retention** (under `~/vl-backups/auto/`):
...
BK=$(ls -1 ~/vl-backups/auto/daily/vl-*.db.gz | sort -r | head -1)
...
- These automated backups are **separate** from the ad-hoc `~/vl-backups/<name>/`
  guarded-write snapshots; both live under `~/vl-backups/`.
