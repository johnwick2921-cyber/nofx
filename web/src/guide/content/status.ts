...
export const status: GuideSection = {
  ...
  blocks: [
    ...
    {
      kind: 'p',
      text: "An entry typed in the agent chat goes through the same chain as an AI decision, plan mode included: under strict it is refused. A chat entry must carry its own stop AND target: without both it is refused when proposed, before you are asked to confirm, and again at the gate ('no explicit stop', counted under entry_gate). With them, the entry gate's R:R and stop-distance checks judge it like any decision, and the order is sent with exactly those prices — never a bracket another decision left at the broker. The reply says what happened: 'refused by the admission gate' only for a gate refusal; 'send failed at the broker' for a broker error; 'OPENED but UNPROTECTED' if the broker opened it and its stop/target then failed to set (set a stop at once). Today a chat entry can only pass the gate on an NT8 (CME) trader: the stop floor's ATR is read from the NT8 bar cache, so on any other venue it is refused ('ATR5m unknown').",
    },
    {
      kind: 'p',
      text: "Before an AI entry, a position NinjaTrader already holds is flattened only when nothing on our books explains it. It is left alone — and the AI entry refused, counted as 'reconcile_owned' — when, for that account, instrument and side, an armed order carrying a broker signal or a sent Picture entry is live; when an open position's entry order is one of those; or when one of them filled in the last two minutes (twice the reconciler's grace) on any trader bound to that account, running or stopped, and is not recorded as a position yet; or when another trader has an open position recorded on that account, instrument and side, however old. A warning names that row. If NinjaTrader no longer holds that trader's position, the row is stale and blocks every AI entry on that side of this account until it is closed. If any of these records cannot be read, the position counts as explained: the AI entry is refused and nothing is flattened. A position nothing explains is flattened as before.",
    },
    ...
    { kind: 'h', text: 'The boot ledger, line by line' },
    {
      ...
      lines: [
        ...
        '🖼 picture rows in flight (the latch\'s + installation gate\'s set, every trader): <none|N — #<id> <stage> submitted_at=<time CT|n/a>; …> · run_epoch=<epoch|n/a> — per trader, READ from the Picture ledger and the live run epoch; a row listed here holds the entry latch ("latched by picture row #<id>")  ← W-EXEC-TRUTH W5',
