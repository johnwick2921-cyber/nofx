...
export const status: GuideSection = {
  ...
  blocks: [
    ...
    {
      kind: 'p',
      text: "On the CME futures trading path — every AI entry, close and entry check — the bot reads only NinjaTrader's own bars and makes no call to any outside market-data service. Open interest and funding are crypto-perpetual ideas with no CME symbol behind them, so on futures they are shown as n/a, never a made-up 0: in the AI prompt (when the Open Interest indicator is switched on) and on the 📊 market data boot line. A NinjaTrader trader set up with a non-CME symbol is refused by the market read rather than sent to a crypto data source, and the boot line names it as REFUSED. Before this, every AI entry, close and entry check asked a crypto exchange's API for MNQ's open interest and funding. That request could never succeed and put a third-party network wait inside an entry decision. On the crypto path open interest and funding now have no data source at all, so the AI decision prompt reads n/a for them, never 0, and the strategy editor says 'No data source' under both toggles; a saved strategy keeps its setting. The chat assistant's background market watcher and its daily briefs were removed on 2026-09-23 together with the crypto exchange that fed them, and no call to that exchange remains anywhere in the app. The chat page's market ticker and the assistant's market snapshot read NinjaTrader bars for CME symbols and say 'unavailable' for anything else: the ticker shows MNQ's last price and its 1-hour and 4-hour change, labelled with NinjaTrader as the source. A change whose window the cached bars do not reach back to reads n/a, never 0, and a symbol it cannot read shows the reason. An exchange account saved with a type that is no longer offered is not converted to another exchange: its traders are refused at load with the type named, and opening it in the exchange settings shows a 'no longer supported' notice with the delete button.",
    },
    { kind: 'h', text: 'The boot ledger, line by line' },
    {
      ...
      lines: [
        ...
        '📊 market data: futures traders=<n> bars=<NT8 BarCache|UNWIRED|n/a> · oi/funding: <n/a (no external market data on the futures path)|n/a (no NinjaTrader trader loaded)>[ · REFUSED on the NinjaTrader venue (non-CME symbol): <symbols>] · non-futures traders=<m> — every field READ: the counts from the loaded traders, bars from whether the NinjaTrader bar feed is wired, oi/funding from the market read\'s own route decision over the symbols those traders trade  ← W-NB A, 2026-09-23 (replaces the old "Using CoinAnk API for all market data" line, which was wrong on futures)',
