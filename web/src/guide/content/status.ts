...
export const status: GuideSection = {
  ...
  blocks: [
    ...
    {
      kind: 'p',
      text: "Inside one trader, one entry is sent at a time: an entry's stop and target are set and the entry is sent under one lock, and the stop and target set again after the send take the same lock. An AI decision and an agent-chat entry sent at the same moment therefore each go out with their own bracket, never each other's. On NinjaTrader the entry also carries its own stop and target into the send, so a send refused before it reached NinjaTrader leaves no bracket behind, and a later breakeven move is judged against the real stop.", // W1b-pending: FOLD-2/3/10 — FOLD-10 entrySendMu (8f7e0a9e) + FOLD-3 OpenWithBracket (dac9d1c9); not at 48707044
    },
    ...
    {
      kind: 'p',
      text: "Where a chat entry goes: a CME symbol typed in the chat — MNQ, ES, NQ and the rest, in any form (MNQ, MNQU6, MNQ.c.0) — is never read as a stock ticker. It goes only to the one running NinjaTrader trader of that instrument, and is refused by name if none is running or if two are: the chat never picks an account for you. A stock whose ticker is also a CME root (CL, ES, NG) is read as the future, and a chat close of a CME symbol goes to that NinjaTrader trader too. So a chat entry can be sent on an NT8 (CME) trader, and only there. It is then sent through the AI decision's own execute path: reconcile-before-open (which, exactly as for an AI entry, may first flatten a position nothing on our books explains), the max-positions limit, the same-side check, and an order row under its signal id, so its fill is tracked like an AI entry's. The contracts you typed are sent as typed or refused — 'refused: quantity N exceeds the max-contracts cap C', or '… is not a whole number of contracts' — never reduced. The order carries your own stop and target, rounded to the tick like every entry (the stop away from the entry). A refusal by those rails replies 'entry refused before any send (the same execute-side rails as an AI decision)'; if an orphan flatten had already been sent, the reply says 'an orphan flatten was sent; the entry was refused'. A refusal inside the NinjaTrader connection (the maintenance hold, the entry latch, an unbound or non-SIM account, the duplicate-order guard) is reported as 'send failed at the broker'. On NinjaTrader the reply waits a few seconds for the fill confirmation, and up to about 35 seconds more when an orphan must be flattened first.", // W1b-pending: FOLD-2/3/10 — FOLD-5 routing (820786e8 + c0dda4c7), FOLD-2 rails + flatten reply (6de15b84, 7ff03ae6), FOLD-3 (dac9d1c9); not at 48707044, where MNQ still routes to 'no running stock trader (Alpaca)'
    },
    {
      kind: 'p',
