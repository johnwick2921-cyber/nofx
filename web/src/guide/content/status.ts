...
export const status: GuideSection = {
  ...
  blocks: [
    ...
    {
      kind: 'p',
      text: "On the CME futures trading path — every AI entry, close and entry check — the bot reads only NinjaTrader's own bars and makes no call to Binance or any other outside market-data service. Open interest and funding are crypto-perpetual ideas with no CME symbol behind them, so on futures they are shown as n/a, never a made-up 0: in the AI prompt (when the Open Interest indicator is switched on) and on the 📊 market data boot line. A NinjaTrader trader set up with a non-CME symbol is refused by the market read rather than sent to a crypto data source, and the boot line names it as REFUSED. Before this, every AI entry, close and entry check asked Binance for MNQ's open interest and funding. That request could never succeed and put a third-party network wait inside an entry decision. On the crypto path open interest and funding now have no data source at all, so the AI decision prompt reads n/a for them, never 0, and the strategy editor says 'No data source' under both toggles; a saved strategy keeps its setting. The chat assistant's background market watcher and its daily briefs were removed in W-NO-BINANCE Part B, and no Binance call remains anywhere in the app. The chat page's market ticker and the assistant's market snapshot read NinjaTrader bars for CME symbols and say 'unavailable' for anything else: the ticker shows MNQ's last price and its 1-hour and 4-hour change, labelled with NinjaTrader as the source, and a value it does not have reads n/a with the reason, never 0. An exchange account saved with a type that is no longer offered is not converted to another exchange: its traders are refused at load with the type named, and opening it in the exchange settings shows a 'no longer supported' notice with the delete button.",
    },
