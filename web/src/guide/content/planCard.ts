    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          body: "The MACHINE's sit-out list, not the plan's prose. Three sources, and they do NOT bind the same way. The first-N-minutes count (kernel.FirstNoTradeMinutes) and the lunch window (kernel.LunchWindowCT) are owner-ruled code constants with no config surface, and they have exactly two consumers: the AI-decision entry gate (trader/auto_trader_orders.go:281) and the adherence grader, which scores after the fact and refuses nothing. Nothing on the ARM path reads them — no band predicate appears anywhere in trader/armed_executor.go — so with plan_mode=strict, where a resting order is the only way into the market, neither band can refuse an entry. The red-news T1 blackouts are the one source that binds both paths, by two mechanisms: the decision gate refuses inside the resolved windows (clock widening and the fail-closed static fallback included), and the T1 force-flat cancels resting arms and flattens open positions from the resolved lead through each window's end. Every window is stamped against YOUR clock when you open the card: live ones are the rules, spent and other-session ones collapse behind a count. The model's own no_trade prose is COLLAPSED behind a 'Model notes' toggle (owner ruling 2026-09-03) — it is a note, never a rule, and it does not render until you ask for it. It was showing the machine's own windows back at you: ASIA v14 carried 'first 5m (CT)' and '12:00-13:30 CT lunch' because the prompt's JSON example demonstrated exactly those. The example is a placeholder now ('<your own sit-out conditions, or omit>'), so the field should carry the model's own reasons or nothing — and it renders nowhere by default either way. Before 2026-09-02 the card rendered that prose verbatim, so an ASIA card at 23:00 CT listed three constraints (first 5m spent six hours earlier, NY lunch, a 09:00 blackout) and not one of them could refuse an entry. A plan written before the band has no machine windows and still renders its prose as rules.",
