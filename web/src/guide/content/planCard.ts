    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          body: '⏳ armed = order authorized, not yet sent · ⏳ placement pending = command registered for sending, awaiting an NT8 receipt · 📌 working = received live entry state from NT8 · ⚡ filled = entry taken (the real fill) · ✕ rejected = received NT8 rejection (verbatim reason when supplied; h1 omits it, so reason unavailable) · cancelled/expired/refused. Command age starts at creation time; cached bar time remains a separate market fact. Payloads older than 60 seconds are refused before sending. The arm is the fast path: it pre-commits the entry so the fill happens at the plan price, not after a 2-minute debate.',
