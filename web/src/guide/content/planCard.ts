    ...
    { kind: 'mockCard' },
    {
      ...
      items: [
        ...
        },
        {
          ...
          body: "The MACHINE's sit-out list, not the plan's prose. Three sources, all of them enforcing: the first-N-minutes and lunch bands come from the same definitions the entry gate refuses on and the adherence grader scores against, and the red-news blackouts are literally the windows the gate will refuse inside (clock widening and the fail-closed static fallback included). Every window is stamped against YOUR clock when you open the card: live ones are the rules, spent and other-session ones collapse behind a count. The model's own no_trade prose still shows, under Model notes — it is a note, never a rule. Before 2026-09-02 the card rendered that prose verbatim, so an ASIA card at 23:00 CT listed three constraints (first 5m spent six hours earlier, NY lunch, a 09:00 blackout) and not one of them could refuse an entry. A plan written before the band has no machine windows and still renders its prose as rules.",
