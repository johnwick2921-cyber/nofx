...
export const guards: GuideSection = {
  ...
  blocks: [
    ...
    { kind: 'p', text: 'When current one-setup permission is unavailable, an older unplaced authorization cannot substitute for it. That authorization is retired; if retirement cannot be saved, the cycle submits no new entry. More generally, production placement requires that the row passed admission and was saved successfully in the current cycle.' },
    { kind: 'p', text: 'A refused stop entry leaves other scenarios available. Once an entry attempt has been registered, its account commitment remains until reconciliation, including when transmission reports an error.' },
    { kind: 'p', text: 'Session and news cutoff handling runs on the tick clock before unchanged-bar or bar-close skips. A quiet tape does not preserve an expired unfilled authorization.' },
    ...
    { kind: 'p', text: 'Private trader requests check every supplied trader ID against the signed-in owner. Order-fill history also verifies the order and fill rows belong to that trader; history remains readable when its execution engine is stopped. A rejected strategy update leaves the saved configuration unchanged. Trader deletion either removes the trader and its equity history together or preserves both when it fails. Ask-Planner historical fallback stays with the selected trader; an empty plan can still show market facts.' },
    { kind: 'h', text: 'Structural stop and first-zone target — research candidate' },
    { kind: 'p', text: 'Planner instructions distinguish reject fades from legacy ATR-floor plays. Authored reject prices do not generate legacy feasibility warnings: admission evaluates the composed structural trade. Omitting an arm does not authorize an AI-path bypass.' },
