...
export const guards: GuideSection = {
  ...
  blocks: [
    ...
    {
      kind: 'p',
      text: "An AI entry becomes a position only when NinjaTrader reports a fill for that exact order. Each entry is matched by its own signal id. A fill records the position at the real fill price. A rejection records the order as rejected, with no position and no 'Filled' alert. No answer within about three seconds leaves the order recorded as submitted and unconfirmed, with a warning, and no position: if it fills later, the position is picked up from NinjaTrader's own position list and tagged with that entry's own signal id from the fill NinjaTrader reported (only fills from the last few minutes count), so breakeven and trailing can move its stop. Its order is marked filled at the real fill price, and the fill is recorded as usual. It never takes on an older armed order's plan link, and it only claims an order still waiting for its fill. If two entries filled on the same side in that window, or the order was already settled, the position stays untagged rather than guess. Before this, an entry that was only queued, or even rejected, was recorded as an open position at the market price, and a fill belonging to an armed or Picture order could be recorded as the AI's.",
    },
    {
      kind: 'p',
