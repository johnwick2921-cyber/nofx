    ...
    <nav className="fixed top-0 w-full z-50 header-bar">
      <div className="flex items-center justify-between h-16 px-4 sm:px-6 max-w-[1920px] mx-auto">
        ...
        >
          ...
          <span className="text-lg font-bold text-vl-gold">VL</span>
        </div>
              ...
              return navTabs
                ...
                .map((tab) => (
                  <button
                    ...
                    ${resolvedCurrentPage === tab.page ? 'text-vl-gold' : 'text-vl-text-muted hover:text-vl-gold'}`}
                  >
                    {resolvedCurrentPage === tab.page && (
                      <span className="absolute inset-0 rounded-lg bg-vl-gold/15 -z-10" />
                    )}
                    {tab.label}
                    {tab.badge && (
                      <span className="ml-1 text-[10px] px-1.5 py-0.5 rounded-full bg-vl-gold/20 text-vl-gold font-semibold uppercase align-top relative -top-1">
                        {tab.badge}
                      </span>
                    )}
                  </button>
          ...
          <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative" ref={userDropdownRef}>
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded transition-colors bg-vl-bg-lighter border border-vl-gold/20 hover:bg-white/5"
                  >
                    <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-vl-gold text-black">
                      {user.email[0].toUpperCase()}
                    </div>
                    <span className="text-sm text-vl-text-muted">
                      {user.email}
                    </span>
                  ...
                  {userDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 rounded-lg shadow-lg overflow-hidden z-50 bg-vl-bg-lighter border border-vl-gold/20">
                      <div className="px-3 py-2 border-b border-vl-gold/20">
                        <div className="text-xs text-vl-text-muted">
                          {t('loggedInAs', language)}
                        </div>
                        <div className="text-sm font-medium text-vl-text-muted">
                          {user.email}
                        </div>
                      </div>
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors hover:bg-white/5 text-vl-text-muted hover:text-white"
                      >
                      ...
                      {onLogout && (
                        <button
                          ...
                          className="w-full px-3 py-2 text-sm font-semibold transition-colors hover:opacity-80 text-center bg-vl-danger/20 text-vl-danger"
                        >
              ...
              resolvedCurrentPage !== 'register' && (
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="px-3 py-2 text-sm font-medium transition-colors rounded text-vl-text-muted hover:text-white"
                  >
                    {t('signIn', language)}
                  </button>
                </div>
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded transition-colors text-vl-text-muted hover:bg-white/5"
              >
              ...
              {languageDropdownOpen && (
                <div className="absolute right-0 top-full mt-2 w-32 rounded-lg shadow-lg overflow-hidden z-50 bg-vl-bg-lighter border border-vl-gold/20">
                  <button
                    onClick={() => {
                      onLanguageChange?.('zh')
                      setLanguageDropdownOpen(false)
                    }}
                    className={`w-full flex items-center gap-2 px-3 py-2 transition-colors text-vl-text-muted hover:text-white
                      ${language === 'zh' ? 'bg-vl-gold/10' : 'hover:bg-white/5'}`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 transition-colors text-vl-text-muted hover:text-white
                      ${language === 'en' ? 'bg-vl-gold/10' : 'hover:bg-white/5'}`}
                  >
                    <span className="text-base">🇺🇸</span>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 transition-colors text-vl-text-muted hover:text-white
                      ${language === 'id' ? 'bg-vl-gold/10' : 'hover:bg-white/5'}`}
                  >
                    <span className="text-base">🇮🇩</span>
        ...
        <motion.button
          ...
          className="md:hidden text-vl-text-muted hover:text-white"
          whileTap={{ scale: 0.9 }}
        >
          {mobileMenuOpen ? (
            <X className="w-6 h-6" />
                  ...
                  return navTabs
                    ...
                    .map((tab, i) => (
                      <motion.button
                        ...
                        ${resolvedCurrentPage === tab.page ? 'text-vl-gold' : 'text-zinc-500'}`}
                      >
                        {resolvedCurrentPage === tab.page && (
                          <motion.div
                            layoutId="active-indicator"
                            className="w-1.5 h-1.5 rounded-full bg-vl-gold"
                          />
                        )}
                        {tab.label}
                        {tab.badge && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-vl-gold/20 text-vl-gold font-semibold uppercase align-middle relative -top-1">
                            {tab.badge}
                          </span>
                        )}
                  ...
                  {isLoggedIn && user ? (
                    ...
                    resolvedCurrentPage !== 'register' && (
                      <button
                        ...
                        className="flex items-center justify-center bg-vl-gold text-black rounded-lg font-bold text-sm hover:bg-yellow-400 transition-colors"
                      >
                        {t('signIn', language)}
                      </button>
                    )
