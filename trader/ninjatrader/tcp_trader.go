...
// handleFill applies one fill to this trader's durable cache. W117 F2: the
// owning ordered consumer runs this ON the read goroutine, before fanout;
// the OrderedHandled flag then makes the channel copy a no-op so evidence is
// applied exactly once, in receive order.
func (t *TCPTrader) handleFill(fill ntwire.FillPayload) {
	if fill.OrderedHandled {
		return
	}
