...
func (at *AutoTrader) snapshotSessionProfiles() {
	...
	nakedPOCProviderOnce.Do(func() {
		...
		at.logInfof("🛑 min-sl guard: atr_mult=%.1f level_clearance=%dtick(s)",
			kernel.MinSLATRMult(), kernel.MinSLTickClearance) // W-WRITE-TIME-FEASIBILITY (2026-09-18) — READ from the resolved knob,
		// never typed.
		at.logInfof("🎛 entry law: write_feas=%s", writeFeasLabel(dp)) // PLAN-LIFECYCLE WAVE (2026-08-27) — hysteresis + dormant/re-arm +
