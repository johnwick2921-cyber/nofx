...
func TestStructuralGeometryBootReadsPolicyAndDurableCounts(t *testing.T) {
	...
	if err := st.GormDB().Exec(`UPDATE strategies SET config=? WHERE id=?`, `{"day_plan":{"structural_stop":{"buffer_points":2.25}},"ai_config":{"risk_control":{"min_risk_reward_ratio":2,"max_trade_loss_usd":{"MNQ":42}}}}`, "99-bound").Error; err != nil {
		t.Fatal(err)
	}
	...
	for _, s := range []string{"buffer=2.25[I]", "owner override; percentile n/a", "refused today=1", "rr<2.00=1", "risk-cap=$42.00"} {
