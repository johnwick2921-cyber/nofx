...
// original durable position identity, so a filled limit or known replacement no-ops.
// The broker close protocol has no position-ID fence: DB lag can still hide a
// broker-side replacement. This check is not an atomic broker identity guarantee.
...
func (at *AutoTrader) finishLimitFlatten(pending *pendingLimitFlatten) {
	...
	for _, po := range open {
		...
		if strings.EqualFold(p.Side, "LONG") {
			_, err = at.trader.CloseLong(p.Symbol, 0)
		} else {
			_, err = at.trader.CloseShort(p.Symbol, 0)
		}
		if err != nil {
			at.logErrorf("%s: fallback close position %d failed; preserving protection: %v", pending.tag, p.ID, err)
			return
		}
		_ = at.trader.CancelStopOrders(p.Symbol)
		return
