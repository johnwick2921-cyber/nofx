...
func (at *AutoTrader) recordLevelStateAt(now time.Time) {
	...
	maxLevels, _, _, _ := resolveSessionPlanCfg(at.dayPlanCfg(), at.activeSessionName(now))
	...
	if _, _, minGrade, _ := resolveSessionPlanCfg(at.dayPlanCfg(), at.activeSessionName(now)); minGrade != "" {
