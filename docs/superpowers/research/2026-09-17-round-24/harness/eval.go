...
type episodeRec struct {
	...
	// ROUND 24 (2026-09-17): the level's own geometry and provenance, so an
	// episode can be placed against HTF zones without a second pass.
	Price    float64 `json:"price"`              // the D1′ anchor (zones: midpoint)
	Lo       float64 `json:"lo"`                 // zone bottom (== price for a line)
	Hi       float64 `json:"hi"`                 // zone top (== price for a line)
	Label    string  `json:"label,omitempty"`    // detector label, e.g. OB(bull), iFVG(bear)
	Polarity string  `json:"polarity,omitempty"` // support | resistance | "" (unknown) — see zonePolarity
	Delta    float64 `json:"delta"`              // Δ used for this read's barriers
...
func runEvalWith(bd *barDB, reads []*readSnapshot, outDir string, s4 *s4State) error {
	// ROUND 24: every HTF zone in each read's RAW universe, touched or not, so an
	// episode can be placed against the zones that EXISTED at the read (lookahead-
	// safe by construction: the read precedes the window by 30 min).
	zFile, zerr := os.Create(filepath.Join(outDir, "zones.jsonl"))
	if zerr != nil {
		return zerr
	}
	defer zFile.Close()
	zonesW = bufio.NewWriterSize(zFile, 1<<20)
	defer zonesW.Flush()
...
func processRead(bd *barDB, r *readSnapshot, w *bufio.Writer, q1, q4 map[string]*cellAgg, s4 *s4State) (nEp, nLevelScans, nSkipped int, err error) {
	...
	if zonesW != nil {
		if err := emitZones(bd, r, delta); err != nil {
	...
	for _, l := range r.Raw {
		...
		for _, e := range eps {
			rec := episodeRec{
				...
				Price: l.Price, Lo: l.Lo, Hi: l.Hi, Label: l.Label,
				Polarity: zonePolarity(bd, r.Contract, tf, l),
				Delta:    delta, ReadAtMs: r.ReadTime.UnixMilli(), Contract: r.Contract,
			}
...
// zonePolarity (ROUND 24) names the side a level is expected to hold as:
//
//	support    — DEMAND, EQL, OB(bull), iFVG(bull), FVG formed as a gap UP
//	resistance — SUPPLY, EQH, OB(bear), iFVG(bear), FVG formed as a gap DOWN
//	""         — every other kind, and an FVG whose formation candle cannot be
//	             found on its own timeframe series (NOT MEASURED, never guessed)
//
// OB and iFVG carry polarity on their detector label (kernel/levels_zones.go);
// a plain FVG does not, so it is read back from the three-candle geometry at
// FormedAtMs on the series the detector ran on: prev.High < next.Low is a gap
// up (bullish → support), prev.Low > next.High a gap down (bearish → resistance).
func zonePolarity(bd *barDB, contract, tf string, l kernel.DetectedLevel) string {
	switch l.Kind {
	case kernel.KindDemand, kernel.KindEQL:
		return "support"
	case kernel.KindSupply, kernel.KindEQH:
		return "resistance"
	case kernel.KindOB, kernel.KindIFVG:
		if strings.Contains(l.Label, "(bull)") {
			return "support"
		}
		if strings.Contains(l.Label, "(bear)") {
			return "resistance"
		}
		return ""
	case kernel.KindFVG:
		...
		i := sort.Search(len(rows), func(k int) bool { return rows[k].openMs >= l.FormedAtMs })
		if i <= 0 || i+1 >= len(rows) || rows[i].openMs != l.FormedAtMs {
			return ""
		}
		prev, next := rows[i-1], rows[i+1]
		if prev.h < next.l {
			return "support"
		}
		if prev.l > next.h {
...
// ROUND 24 — the per-read HTF zone universe.
var zonesW *bufio.Writer
...
var r24ZoneKinds = map[kernel.LevelKind]bool{
	kernel.KindOB: true, kernel.KindFVG: true, kernel.KindIFVG: true,
	kernel.KindSupply: true, kernel.KindDemand: true, kernel.KindEQH: true, kernel.KindEQL: true,
}
var r24ZoneTFs = map[string]bool{"15m": true, "1h": true, "4h": true, "1d": true}

func emitZones(bd *barDB, r *readSnapshot, delta float64) error {
	for _, l := range r.Raw {
		if l.Price <= 0 || !r24ZoneKinds[l.Kind] || !r24ZoneTFs[l.TF] {
			continue
		}
		rec := zoneRec{Day: r.Day, Session: r.Session, ReadAtMs: r.ReadTime.UnixMilli(), Contract: r.Contract,
			Kind: string(l.Kind), TF: l.TF, Price: l.Price, Lo: l.Lo, Hi: l.Hi, Label: l.Label,
			Polarity: zonePolarity(bd, r.Contract, l.TF, l), FormedAtMs: l.FormedAtMs, Delta: delta}
		line, err := json.Marshal(rec)
		if err != nil {
			return err
		}
