...
**[A] Wang, Wu and Chung (2015), PEER-REVIEWED conference paper:** TAIEX one-minute data, **4 January 2010–25 March 2015; 1,291 trading days and 1,180 trades per main variant**. Their momentum strategy tests 17 fixed stops from 20 to 100 points, in five-point increments. A 30-point stop raises mean gross profit from approximately 4.24 to 5.09 points and reduces maximum drawdown from 1,234 to 659 points. **Costs are excluded. Transfer: indirect only; momentum on TAIEX is not a Nasdaq fade, and neither the numerical distances nor the optimum transfers.**[^7]

**[A] Howard (2026), VENDOR-OR-SELF-REPORTED empirical preprint:** ES value-area **breakout continuation**, **January 2025–January 2026; 6,284 events, one-second data**. The abstract reports that all 12 tested price-stop distances reduce value relative to its unmanaged horizon; a combined time-aware exit stack outperforms the price-stop baseline in 11 of 13 months. The stack includes trailing, break-even and no-progress components, so this is not an isolated time-stop experiment. **Transfer: a useful counterexample to universal stop prescriptions, not an MNQ fade calibration.** Full methodology/code were not available in the inspected abstract; costs, exact stop grid and independent holdout could not be verified.[^8]
...
**Practitioner specificity.** Carter's public intraday futures page describes pivot retracement limit entries, including a **0.50-point NQ entry offset**, and also lists **ES three-point stop/two-point target/35-minute timeout** management. It does not publish an audited sample or a corresponding NQ stop distance. Its separate stock/options swing plan uses daily ATR, partials and Fibonacci extensions. **VENDOR-OR-SELF-REPORTED** rules embedded in a commercial page; unsupported success-rate language is **MARKETING**. **Transfer: candidate mechanics only, not the historical distances or success claims.**[^5]
...
**[T] Compare three full-contract exits with identical initial entry/stop:** first structural target; a prespecified closed-bar trail; and a prespecified timed/no-progress exit with hard protection. Also compare fixed-R controls. Keep stop movement and re-entry disabled in the primary structural-target arm. The Howard study motivates testing an exit stack, but cannot establish which component or which exit is best for a mean-reversion book.[^8]
