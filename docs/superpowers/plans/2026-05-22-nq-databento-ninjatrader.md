...
> **STATUS (2026-06-02, latest — Strategy Studio UNIVERSAL FE gating + BE-2 SHIPPED):**
> branch `feat/nt8-stage4-chart` tip `c9307952`. The Strategy Studio page is now
> futures-universal (additive; crypto byte-identical; nothing deleted). Five
> Tier-1 FE gates SHIPPED, gated by `isCMEFutures(static_coins[0])`:
> (1) Risk Control — the Position-Value-Ratio "BTC/ETH + Altcoin" residual is now
> hidden for futures and replaced by a Futures Sizing panel (Notional Ceiling =
> equity × 20 + contract-sizing note); (2) Preview/AI-Test send `prompt_variant:
> 'futures'` for a futures strategy (the desk now previews the REAL futures system
> prompt — live-verified "CME … MNQ … $2/point", zero crypto framing — instead of
> the crypto prompt); (3) Coin Source shows only the Static List source for futures
> (AI500/OI tiles + NofxOS panels hidden); (4) Indicators hides the whole NofxOS
> crypto data block (AI500/OI-Ranking/NetFlow/Price-Ranking + the `cm_568c67` key);
> (5) the "AI Grid Trading" strategy-type option is hidden for futures (grid is
> crypto-CEX-only; NT8 bridge is market-only). **BE-2 (R/R gate) SHIPPED** — the
> risk gate now threads the strategy's `MinRiskRewardRatio` and honors it on the
> futures branch (crypto keeps 3.0 byte-identical). **Premise refutation (proven by
> `TestRRGate_SyntheticEntryIsAlways4to1`):** the gate derives R/R from a synthetic
> 20%-from-SL entry, so measured R/R is a constant 4.0:1 for any valid open — the
> old hardcoded 3.0 floor NEVER rejected a futures trade. So BE-2 is
> config-authoritative *hygiene* (future-proofs if the entry model ever uses the
> real entry), NOT a trade-unblocking fix; the change-map's "silently rejects 1.5-RR
> futures trades" claim is corrected here. Source-of-record for the full
> section-by-section map + remaining work:
> `docs/superpowers/reports/2026-06-02-strategy-studio-universal-futures-map.md`.
> **Still remaining (Tier-2 backend gaps, NOT yet built):** BE-1 live prompt-variant
> keyed on broker not symbol; BE-3 futures default seed env-gated (`TRADING_MODE`),
> not per-strategy; BE-4 `BuildFuturesDecisionSystemPrompt` ignores `PromptSections`;
> BE-5 preview/test-run `config_summary` + `BuildUserPrompt` crypto framing; BE-6
> save-time `ClampLimits`/warnings + the "USDT" min-size error string. **Grid editor
> + engine are PARKED** (crypto-only; no NT8 limit-order execution path). FE-only
> verified in a real browser (MNQ futures UI + crypto byte-identical); `go build`
> + kernel gate/R-R tests pass (pre-existing date-sensitive `TestMaybeResetDaily`
> unrelated). New Round at end of doc.
>
...
# 2026-06-02 Round — Strategy Studio Universal (Futures) FE gating + BE-2 SHIPPED (`c9307952`)

**Source-of-record:** `docs/superpowers/reports/2026-06-02-strategy-studio-universal-futures-map.md`
(the full section-by-section crypto→universal change-map, committed `423396df`).

**Frame (unchanged):** additive only — KEEP crypto byte-identical, ADD the futures
path, DELETE NOTHING. Every FE gate keys off `isCMEFutures(static_coins[0])`
(`web/src/lib/instrument.ts`); no new hardcoded symbols/$ values.

## SHIPPED — Tier-1 FE gating (5 items)

1. **Risk Control — Position-Value-Ratio residual fixed.** `RiskControlEditor.tsx`:
   the "BTC/ETH Position Value Ratio" + "Altcoin Position Value Ratio" block
   (`:154-209`) is now wrapped `{!isFutures && …}`; for futures a read-only
   **Futures Sizing** panel renders instead — Notional Ceiling = equity × 20 (the
   gate's `futuresMaxNotionalLeverage`, `engine_position.go:15`) + a contract-sizing
   note. New i18n `futuresSizing*`/`futuresNotionalCeiling*`/`futuresContractSizing*`
   (en/zh/es). The Go gate already bypassed these ratios for futures — FE-only fix.
2. **Preview / AI-Test fidelity.** `StrategyStudioPage.tsx`: `fetchPromptPreview` +
   `runAiTest` send `prompt_variant:'futures'` when the strategy symbol is CME
   futures; the variant `<select>` shows a single "Futures (NQ/MNQ)" option for
   futures (3 crypto tones unchanged). Live-verified: an MNQ strategy now previews
   the futures system prompt ("professional CME index-futures trading AI … MNQ …
   $2.00 per index point"), not the crypto prompt. No backend change (the futures
   variant + previewSymbol wiring predate this).
3. **Coin Source.** `CoinSourceEditor.tsx`: threaded `isFutures`; for futures only
   the Static List source tile renders (AI500 / OI Increase / OI Decrease tiles +
   their NofxOS panels gated `!isFutures`), placeholder → "MNQ, NQ, ES…".
4. **Indicators.** `IndicatorEditor.tsx`: the entire NofxOS crypto data-provider
   block (AI500 / OI-Ranking / NetFlow / Price-Ranking + the dead `cm_568c67` key)
   is gated `{!isFutures && …}`. Market Data + Technical Indicators + Market
   Sentiment stay. (funding-rate toggle was already gated in the capstone.)
5. **Strategy-Type selector.** `StrategyStudioPage.tsx`: the "AI Grid Trading"
   button is hidden for futures (grid is crypto-CEX-only; the NinjaTrader bridge is
   market-only — no grid execution) + a `gridTradingFuturesUnsupported` note; the
   selector collapses to single-column AI Trading. New i18n (en/zh/id).

## SHIPPED — BE-2 (R/R gate) + a premise refutation

`validateDecision` / `validateDecisions` / `parseFullDecisionResponse`
(`kernel/engine_position.go`, `engine_analysis.go`) now thread the strategy's
`RiskControlConfig.MinRiskRewardRatio`. On the **CME futures branch** the R/R floor
comes from config; **crypto keeps the hardcoded 3.0 byte-identical**.

**Premise refutation (proven by `TestRRGate_SyntheticEntryIsAlways4to1`):** the gate
derives R/R from a *synthetic* entry placed 20% from SL toward TP, so the measured
R/R is a **constant 4.0:1** for any structurally-valid open (long or short). The old
hardcoded `3.0` floor therefore **never rejected** a valid futures (or crypto) trade.
So BE-2 is **config-authoritative hygiene** — it future-proofs the gate if the entry
model is ever changed to use the AI's real entry — **not** a trade-unblocking fix.
The change-map report's "silently rejects 1.5-R/R futures trades" framing is
corrected by this finding.

## Verification

- `go build ./...` clean; `go vet ./kernel/` clean; kernel gate/R-R tests pass
  (`TestFuturesGate_*`, `TestRRGate_*`, `TestValidateDecisionFormat`). The only
  failure is the pre-existing, date-sensitive `TestMaybeResetDaily` (unrelated — it
  touches daily P&L reset, not the gate). `tsc --noEmit` clean.
- Binary rebuilt + restarted (`go build -o nofx-bin .`); NT8 frames flowing, 0
  "unknown frame type". No C# / wire-frame change (none needed).
- Real browser at HEAD `24634b5a` → committed `c9307952`: **MNQ** shows the futures
  sizing panel / no PVR labels / Static-only coin source / no NofxOS block / no Grid
  option / futures preview prompt. **BTCUSDT** scratch shows leverage tiers, PVR,
  USDT, funding-rate, AI500/OI tiles, NofxOS block, Grid option, 3 tone variants —
  byte-identical. (SIM-only; the scratch BTCUSDT edit was never saved.)

## Still remaining — Tier-2 backend gaps (NOT built this round)

- **BE-1** live `promptVariant` is chosen by `exchange=="ninjatrader"`
  (`auto_trader_loop.go:114`), not by the symbol — FE (`isCMEFutures`) and BE use
  different signals.
- **BE-3** futures default coin seed is env-gated (`TRADING_MODE=futures`,
  `store/strategy.go:935-953`), not per-strategy → can't create a futures strategy
  from the UI in crypto mode.
- **BE-4** `BuildFuturesDecisionSystemPrompt` ignores `PromptSections` (futures
  users' prompt-section edits are inert).
- **BE-5** preview/test-run `config_summary` (leverage rows) + `BuildUserPrompt`
  (BTCUSDT lookup, USDT labels) + the crypto market-wide data fetch in test-run.
- **BE-6** save-time `ClampLimits` + leverage warnings + ai500 default-fallthrough
  ungated for futures; the min-size execution-error string still says "USDT".

## Parked

- **Grid editor + engine** (`GridConfigEditor.tsx`, `trader/auto_trader_grid*.go`,
  `kernel/grid_engine.go`) — crypto-only; the NinjaTrader trader has no
  `PlaceLimitOrder` (market-only, no `CloseLong/CloseShort`), so a futures grid
  cannot execute. Universalizing grid is an execution-engine project, out of scope.
