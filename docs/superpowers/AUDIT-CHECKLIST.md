...
## CLASS NN (assigned at merge) — a guard that reads a field its own caller never copied (OWED)
...
**Owed:** load `State`, `StopPx`, `TargetPx`, `SignalID` from the matched `existing[i]` (or delete the guard and its log line), with a call-site test that a working row whose plan moved the stop ≥ 2 ticks sends exactly one bracket modify.

## CLASS NN (assigned at merge) — a working order that outlives the plan version that priced it (OWED)
...
## CLASS NN (assigned at merge) — a pass-clocked path that builds a wall-clock resolver (OWED)
