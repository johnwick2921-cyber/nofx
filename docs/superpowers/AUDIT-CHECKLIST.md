...
**Known residual [A]:** after a restart empties the ring, the price-match fallback still has no time bound. The ring's account and instrument-root filters are defence in depth (the router `dispatchToOwner` already routes fills by symbol and account) and survive mutation. The `NO recoverable lineage` WARN in `reconcile.go` still fires before the stamp (log only).
...
**Named behaviour, fail-closed:** chat entries currently CANNOT be sent on non-CME venues — `agentBracketRefusal` reads ATR5m through `armSeamATR5m`, which serves only the NT8 BarCache, so an Alpaca or crypto symbol is refused "ATR5m unknown" [A code read]; and the chat's symbol routing does not reach the NT8 door today [B, map probe], so no chat entry can be sent on any venue right now. The non-CME open-then-set branch is therefore unreachable end-to-end and is tested only at `sendManualEntry`. **Still open:** the chat's Set→Open and the AI's Set→Open write the same broker SL/TP map keys without a shared lock [B] — the fix is a broker-level `OpenWithBracket` (phase 2); `parseTradeCommand` builds a bracket-less open but has no production caller [A: grep].
