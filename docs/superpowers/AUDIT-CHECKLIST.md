...
*Highest occupied class: **264** (2026-09-24). Numbers are assigned AT MERGE and
...
**Known limits:**
- The ha2 verifier's compiling revert `id := ident; go func(…){ agents := id.agents … }` brings the race back with both structural pins green: the pin tracks the identity by variable NAME, and an alias escapes it [A]. [PENDING hc: pin tightened to the type in the hc fold.]
