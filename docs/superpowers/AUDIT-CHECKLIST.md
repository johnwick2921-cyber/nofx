...
## CLASS NN (assigned at merge) — a boot cleanup that deletes rows of a type it no longer knows

**Found:** 2026-09-23, W-NO-BINANCE Part B map (`wf_f0d960e4-9ee`, Go slice, D1), ruled by the CTO [A].
**Shape.** `store/exchange.go`'s boot-time cleanup (run from `initTables`) deleted every exchange row whose type fell to the credential validator's default branch. Removing a broker from the validator's list is an ordinary code change, yet it would have turned the next boot into a `data.db` write that DELETED the owner's legacy rows. The data loss would have been silent and irreversible, triggered by a deploy rather than by anyone deciding to delete data. The rows' traders would simply have vanished instead of being shown as refused.
**Probe:** grep every boot and migration path for DELETE/UPDATE statements keyed on a classification (type, state, version) that code changes can move. For each, ask what happens to a row the classifier stops recognising. The answer must be "left alone, and reported". Deletion requires an explicit, named, owner-run migration.
**Fixed in W-NO-BINANCE B:** the cleanup never deletes or modifies a row because its type is unsupported; the row stays, and its trader is REFUSED at load with the type named. The boot loader records the refusal the way the user loader does. Pinned at the production call sites: `TestBootCleanupLeavesAnUnsupportedExchangeRowUntouched` (store close + reopen runs `initTables`; the unsupported and untyped rows stay byte-identical while supported controls are still cleaned) and `TestLegacyRemovedExchangeRowsSurviveBootAndTheBootLoaderRecordsTheRefusal` (the boot loader `LoadTradersFromStore`).
