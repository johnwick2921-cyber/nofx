...
71. **A non-agent writer mutating the deploy tree — locks govern agents, not
    ...
    `IncArmRefusal` + `IncStopUnanchored` (0B), `normalizeArmLegs` +
    `ArmNormalizations` (class 39), and the real `GetOpenOrders` reverted to its
    stub (class 33). It went unnoticed for **3h20m** and was found by accident,
    not by any check.
    **No agent did it and no git command did it.** The six files came from three
    different points in history — no commit in the last 120 matched more than 4
    of 6 — and VS Code's local history holds entries byte-identical to the lost
    content, written at 08:46:33 and 08:46:34. That is a save, not a checkout.
    **This is why the worktree law and `~/nofx-main.lock` could not have helped.**
    Both govern AGENTS. An editor is not an agent, holds no lock, reads no
    protocol, and its write is indistinguishable from a legitimate one until
    someone reads the content. Every control in place was aimed at the wrong
    actor.
    **Probe:** for any invariant enforced by convention among agents, ask what
    else can write to the same place. A human with an editor, a sync client, a
    backup restore, an IDE refactor across a symlink — none of them read your
    lock file. Then ask the second question: **if that write happened right now,
    what would tell you?** If the answer is "someone would notice eventually",
    the invariant is undefended.
    **Law:** you cannot lock out an actor that does not participate in the lock.
    What you can do is DETECT, and detection must not depend on the same actor
    being honest. The mitigation shipped as `deploy/nofx-tree-guard.sh` — four
    read-only checks on a 60 s timer, alarming on a dirty tree with no live lock
    holder, on a missing shipped symbol (which catches the same loss *after* a
    commit, when porcelain would call it clean), on RELEASE disagreeing with the
    running binary, and on an unpushed HEAD. **It observes and never repairs**: a
    repairing guard would be a second unaccountable writer, which is the disease
    rather than the cure. It shortens the discovery window from hours to a
    minute; it does not close the hole. The only real fix is to stop opening the
