...
tightening 1790178967603; see the identity≠price CLASS NN at the end of this
...
the CLASS NN "a write-time check that judges only the read-time price or the
...
## CLASS NN (assigned at merge) — a third-party market-data call on the trade path, keyed on the symbol instead of the venue
...
## CLASS NN (assigned at merge) — a numeric field that cannot say "absent", so the code writes a fabricated 0
...
## CLASS NN (assigned at merge) — a precedence stated in prose, implemented by the append order of a first-match scan
...
## CLASS NN (assigned at merge) — a window named by a duration but measured by a bar count (or by the previous bar)
...
## CLASS NN (assigned at merge) — a drift counter that skips the fields a custom MarshalJSON re-emits
...
## CLASS NN (assigned at merge) — a clamped copy stands in for the saved value
...
## CLASS NN (assigned at merge) — a UI fallback literal names a default the runtime does not use
...
## CLASS NN (assigned at merge) — a 0 that means "unset" to one reader and "off" to another
...
## CLASS NN (assigned at merge) — a record that vouches for a row is written outside the row's transaction
...
## CLASS NN (assigned at merge) — a stored rule evaluated with an env default
...
## CLASS NN (assigned at merge) — a close count that trusts the order of its input
...
## CLASS NN (assigned at merge) — a test file silently excluded by a GOOS/GOARCH filename suffix
...
## CLASS NN (assigned at merge) — an unparseable safety sentence accepted as UNKNOWN
...
## CLASS NN (assigned at merge) — a write-time check that judges only the read-time price or the latest window
...
## CLASS NN (assigned at merge) — an identity that names one level while the scenario trades another, published as a "recorded" finding
...
## CLASS NN (assigned at merge) — a first obstacle that skips a seated level, and a path nobody had to list
...
## CLASS NN (assigned at merge) — a prompt header that tells the model the opposite of what the gate does
...
## CLASS NN (assigned at merge) — an order policy stamped after the validator that refuses it
...
## CLASS NN (assigned at merge) — a price range judged at one point of the range
...
## CLASS NN (assigned at merge) — a re-arm loop that re-places a filled order within the same plan version
...
## CLASS NN (assigned at merge) — a second pass trigger over state guarded only by "run-loop goroutine only"
...
## CLASS NN (assigned at merge) — a new trigger that places through the send point without the authoring gates
...
## CLASS NN (assigned at merge) — a guard that reads a field its own caller never copied (fixed in W1b)
...
## CLASS NN (assigned at merge) — a working order that outlives the plan version that priced it (fixed in W1b)
...
## CLASS NN (assigned at merge) — a producer with its own send path trades behind "No plan"
...
## CLASS NN (assigned at merge) — a new row kind every "latest row" reader already interprets
...
## CLASS NN (assigned at merge) — a failure reported after the durable effect
...
## CLASS NN (assigned at merge) — a plan reader that predates the ONE fold (OWED)
...
## CLASS NN (assigned at merge) — entry permission that outlives the run or the switch that granted it
...
## CLASS NN (assigned at merge) — a single-source rule applied to a second source
...
## CLASS NN (assigned at merge) — a pass-clocked path that builds a wall-clock resolver (fixed in W1b)
...
## CLASS NN (assigned at merge) — a settlement half gated behind the switch whose OFF it must settle
...
## CLASS NN (assigned at merge) — a reconcile sweep that annotates a row another sweep owns
...
## CLASS NN (assigned at merge) — a version-insensitive ledger key under a version-local id mint
...
## CLASS NN (assigned at merge) — a re-spec gate that compares the COMPOSED wire price, so drift reads as a plan change
...
## CLASS NN (assigned at merge) — ownership state keyed by a shared id, cleared by a bare Delete (second instance)
...
## CLASS NN (assigned at merge) — a loud log at a layer with no identity to dedupe on
...
## CLASS NN (assigned at merge) — a price-similarity lineage with no time bound adopts a stale identity while exact signal evidence sits unread
...
## CLASS NN (assigned at merge) — a running-process registry used as the scope of a ledger question
...
## CLASS NN (assigned at merge) — a floor judged on continuous prices, sent on the tick grid by a side-blind rounding
...
Since FOLD-3 the NT8 send carries its own bracket (`TCPTrader.OpenWithBracket`; `placeEntryWith` reads the map only when no bracket is carried) and since FOLD-10 one entry-send section spans set → open for every map-keyed broker. <!-- W1b-pending: FOLD-2/3/10 — OpenWithBracket (dac9d1c9) and entrySendMu (8f7e0a9e) are not at 48707044 -->

## CLASS NN (assigned at merge) — a dedupe slot recorded at admit is consumed by refusals that never reached the wire
...
## CLASS NN (assigned at merge) — a fail-open leg on a path that never supplies its input (abstain = permanently off)
...
**One truth since W1b FOLD-5 (the same story as the guide, `web/src/guide/content/status.ts`):** a CME chat symbol reaches the NT8 door. `isStockSymbol` asks the CME predicate (`isCMEFuturesChatSymbol` = `market.IsCMEFuturesSymbol` or a non-empty `market.FuturesRoot`) BEFORE the letters heuristic, and `resolveCMETrader` (`agent/trade.go`) picks the ONE running NinjaTrader trader whose wire instrument has that root — refused by name when none is running or two are (the chat never picks an account). So a chat entry can be sent on an NT8 (CME) trader, and only there: MNQ reaches `OpenManualEntry` [A at the fold: `TestChatMNQEntryReachesTheNT8TraderDoor`, through `resolveTradeExecutionContext` → `executeTradeWith` → `OpenManualEntry` on an NT8 fake]. <!-- W1b-pending: FOLD-2/3/10 — FOLD-5 is 820786e8 (+ repair c0dda4c7), not at 48707044; at 48707044 MNQ still routes to "no running stock trader (Alpaca)" --> The non-CME open-then-set branch stays unreachable end-to-end and is tested only at `sendManualEntry`. **Still open at the E9 merge — folded in W1b:** the chat's Set→Open and the AI's Set→Open wrote the same broker SL/TP map keys without a shared lock [B at E9; CTO-verified as FOLD-10 [A]] — closed by FOLD-3 (`OpenWithBracket`: the NT8 send carries its own bracket) and FOLD-10 (one entry-send section per trader), classes below. <!-- W1b-pending: FOLD-2/3/10 --> `parseTradeCommand` builds a bracket-less open but has no production caller [A: grep].
...
## CLASS NN (assigned at merge) — a flat window enforced only by a cancel: a second trigger re-arms what the first cancelled
...
## CLASS NN (assigned at merge) — a pin overdetermined by a second guard
...
## CLASS NN (assigned at merge) — a log line that states an attribution the code never established
...
## CLASS NN (assigned at merge) — an identity tagged before the record it names is settled
...
## CLASS NN (assigned at merge) — a freshness window measured against a `now` captured before its reads
...
## CLASS NN (assigned at merge) — a per-pass sweep that re-sends an unpaced cancel
...
## CLASS NN (assigned at merge) — a WIP snapshot taken while a mutation probe runs commits the mutation
...
## CLASS NN (assigned at merge) — a side door that re-implements the send skips the producer's execute-side rails
...
**Fixed in W1b FOLD-2 (+ repairs):** the door sends through the AI decision's own execute path: `sendManualEntry` → `executeOpenLong/Short` (manual) → `openEntryWithRecord`, with a Decision carrying the chat's SL/TP. That runs reconcile-before-open, the max-positions limit and the same-side check. On CME a typed quantity is REFUSED, never clamped ("refused: quantity N exceeds the max-contracts cap C" / "refused: quantity N is not a whole number of contracts") — the owner typed that number. It writes an order row through `recordAndConfirmOrderAs` under the chat's signal id, never taking the AI's plan citation. A rail refusal is typed `ManualEntryRefusal{Execute: true}`, and the chat replies "entry refused before any send (the same execute-side rails as an AI decision): …". When reconcile had ALREADY sent an orphan flatten, the refusal carries `FlattenSent` and the reply says "an orphan flatten was sent; the entry was refused (…)", never "nothing sent". A chat position's excursion row carries the chat's OWN stop/target, never a nearby AI decision's (`StopTargetNear`). Pinned at `OpenManualEntryAt` (a fixed admission clock; `OpenManualEntry` itself is wall-clocked): `TestChatEntryOverTheMaxContractsCapIsRefusedNeverClamped`, `TestChatEntryIsReconciledBeforeOpenLikeAnAIEntry`, `TestChatEntryAtMaxPositionsIsRefusedLikeAnAIEntry`, `TestChatEntrySendIsRecordedUnderItsSignalID`, `TestChatEntryOnAnAlreadyHeldSideIsRefusedLikeAnAIEntry`, `TestChatNonCMEEntryOnTheNT8VenueIsRefusedBeforeAnySend`, `TestChatPositionExcursionCarriesItsOwnBracketNeverAnAIDecisions`, `TestChatEntryRefusedAfterAnOrphanFlattenSaysTheFlattenWasSent`; agent `TestChatEntryExecuteRailRefusalSaysSo`, `TestChatEntryRefusalAfterAnOrphanFlattenSaysTheFlattenWasSent`. <!-- W1b-pending: FOLD-2/3/10 — FOLD-2 is 0339f2c3/6de15b84 + repairs f66aaada, 7ff03ae6 (f2 worktree), not at 48707044; re-grep every test name after the cherry-pick -->
...
**Residuals (named):** (1) [B] on NT8 the door blocks its HTTP goroutine for up to ~35 s of reconcile (`reconcileFlattenTimeout`) plus ~3 s of confirm polling. (2) [A] E15 logs a late chat fill as a "late AI fill": order rows carry nothing that tells a chat order from an AI one. (3) [B] the hop `executeTradeWith → OpenManualEntry` is covered only by fakes; a clock seam for `OpenManualEntry` would let the literal ruling test run without failing in the CME halt hour. <!-- W1b-pending: FOLD-2/3/10 — residuals of the FOLD-2 door (6de15b84 + repairs), not at 48707044 -->

## CLASS NN (assigned at merge) — shared state written before a send that can fail, then read as truth
...
**Residual [C] (FOLD-2 verifier N5):** the maps are written AFTER `SendSignal`, so for a sub-millisecond window `MoveStopToBreakeven` can read the previous value (a 0 skips the widen ban). SYSTEM-MAP's note that `SetStopLoss` "writes a local map a later placeEntry reads" now holds only for the legacy OpenLong/OpenShort path. <!-- W1b-pending: FOLD-2/3/10 — residual of OpenWithBracket (dac9d1c9), not at 48707044 -->

## CLASS NN (assigned at merge) — a shape heuristic asked before the domain predicate
...
**Fixed in W1b FOLD-5 (`820786e8`, repair `c0dda4c7`):** `isStockSymbol` asks `isCMEFuturesChatSymbol` (`market.IsCMEFuturesSymbol` or a non-empty `market.FuturesRoot`) FIRST. `chatTradeSymbol` canonicalizes a CME chat symbol to its root where it enters (canon 28). `resolveCMETrader` (`agent/trade.go`) picks the ONE running NinjaTrader trader whose `WireSymbol()` has that root; none, or two, is refused by name. Named consequence: a CME root that is also a stock ticker (CL, ES, NG) now routes only to NT8 — Colgate cannot be traded through chat, fail-closed. A chat close of a CME symbol now reaches the NT8 trader too (it used to go to Alpaca). Pinned: `TestChatMNQEntryReachesTheNT8TraderDoor` (execute_trade → confirm → `resolveTradeExecutionContext` → `executeTradeWith` → `OpenManualEntry` on an NT8 fake), `TestChatCMEEntryResolvesOnlyTheOneNT8TraderOfItsInstrument`, `TestCMESymbolsAreNeverStocks`, `TestChatCMEResolverSelectsTheNT8TraderThroughTheProductionAdapter` (the production roster adapter, plus a compile-time `var _ interface{ WireSymbol() string } = (*ntTrader.TCPTrader)(nil)`). <!-- W1b-pending: FOLD-2/3/10 — FOLD-5 is 820786e8 + c0dda4c7 (f2 worktree), not at 48707044 -->
...
## CLASS NN (assigned at merge) — a set→send pair on shared state with no lock spanning it
...
**Fixed in W1b FOLD-10 (`8f7e0a9e`):** ONE AutoTrader-level entry-send section, `entrySendMu`, taken only in `openEntryWithRecord` through `lockEntrySend` (idempotent release, deferred for every early return and panic, and called where the section ends). Section 1 spans the pre-entry bracket set → the open send (released as soon as the send returns; the order-confirmation poll runs outside it). Section 2 covers the post-open `SetStopLoss` / `SetTakeProfit`, which on a map-keyed broker would otherwise land inside ANOTHER entry's set → read. Both paths take it: the AI decision's and, since FOLD-2, the chat door's. The Trader interface is unchanged (19 methods). On the live NT8 path, FOLD-3's `OpenWithBracket` already carries each entry's own bracket; the section closes the class for every map-keyed broker (legacy NT8 `placeEntry`, the CSV transport). Pinned: `TestConcurrentAIAndChatEntriesEachSendTheirOwnBracket` (production call sites `executeOpenLongWithRecord` + `OpenManualEntryAt`, a broker that records the maps' (sl, tp) AT OPEN and forces the interleave deterministically, both orders). RED at base ("AI sent (SL 28983.25, TP 29078.25) want (28981.25, 29080.25)") and on both compiling reverts (both sections off; post-open section only). <!-- W1b-pending: FOLD-2/3/10 — FOLD-10 is 8f7e0a9e (built on 6de15b84), not at 48707044 -->
