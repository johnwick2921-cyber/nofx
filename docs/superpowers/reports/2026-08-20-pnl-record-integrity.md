...
- **V4 [A]:** full `go test ./store/ ./trader/ ./trader/ninjatrader/ ./kernel/` green pre-deploy; **30-min post-deploy soak (07:39–08:09 CT) CLEAN** — 13 decision cycles at normal cadence, zero panics, zero `pnl_integrity MISMATCH` (no closes occurred in-window; the guard is unit-pinned instead), zero new WARN classes. Known/expected only: dead-man watchdog DOWN→UP reconnect at 07:41 (the kill -9 restart, entries correctly blocked until clean reconciliation), cycle-#1 `no balance frame yet` defer, x402 payment retries (pre-existing, unrelated). **The load-bearing soak catch: the phantom `daily loss would trip (realized today=-1589.00)` line is GONE** — the soft-audit now fires only `max daily trades would trip (today=5, max=3)`, which is TRUE on real counts (5 trades did close this session-day). The corrected sum (−129.93, inside the −450 limit) demonstrably feeds the guardrail reader live.
