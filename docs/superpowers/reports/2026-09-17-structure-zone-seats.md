...
- One-setup's live seam (`trader/one_setup_wiring.go:131`) and the card's `planW3Map` build their maps without HTF extras (pre-existing); a ZONE seat is therefore visible there only through the plan doc's own levels. Out of scope ("nothing else changes"); flagged for the owner.
