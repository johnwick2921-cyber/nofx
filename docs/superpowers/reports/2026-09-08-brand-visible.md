...
### Attended watch: 14:03–14:45 CT

[A] At 14:03:55 CT, rows 134/135 had become `superseded`; arm 133 remained `working`. Subsequent read-only observations continued to find its matching broker order working through snapshot **14543**. At 14:44:55 CT the broker emitted **14544** (`CancelPending`) and **14545** (`CancelSubmitted`) for `db758fae647843aa93b691bc8ffc6df6`, followed by empty book **14546**. Later frame **14553** was empty as well. This lane sent no cancel or flatten; these observations do not establish who initiated that transition.

[A] The owner-authenticated gate at **14:45:13 CT** returned HTTP 200, `ready=false`: positions legs 1/2/3 PASS and planner leg 5 PASS, but leg 4 **broker 0 / ledger 1 MISMATCH**, citing signal `5d4716c8-298a-4f86-bc72-4d6529d73483`. Row 133 still read `working`; cancel_requested_at_ms/cancel_attempts/cancel_settled_snapshot_id were all **0**, and updated_at remained `2026-09-09 18:20:15.011757027+00:00`. Thus the physical book cleared, but the five-leg gate had not passed. No override, repair, swap or boot by this lane occurred.

[A] In parallel, dev advanced to **8dfe6bc1a7ba219de626cded9dddedc74dc0214c**, merge of `fix/truthful-self-description`, and the lock reported holder **nofx-6b** with task `W5 boot: truthful self-description (merge/build now, cutover at 14:45 CT)`, heartbeat fresh. Dispatch 102 did not take that lock. The owner was asked to resolve combined-cutover ownership; no answer is inferred from elapsed time. The existing 05125bd6 candidate predates those newly merged prompt/Guide changes and cannot be presented as a build of current dev. Any 102-owned cutover must integrate and validate the current merged head first.

[A] **First observed all-pass gate: 2026-09-09 14:46:36 CT.** This lane's own authenticated GET returned HTTP 200 and `ready=true`: DB `0 open row(s)`, API `0 position(s)`, NT8 `count=0`, broker `0 working order(s) at the broker (ledger agrees: 0)` with snapshot age **1s**, and `no planner read claimed`. A read at 14:46:18 CT found row **133 = cancelled**, with empty snapshot **14555**. The process still reported revision **954f11b15f2e** at 14:46:21 CT. This establishes that 133 became terminal and the book cleared before any observed new boot. The fresh main lock remained held by **nofx-6b**; 102 did not seize it or swap. This pass is timestamped evidence, not permission to reuse a stale gate later.
