...
**Stop-entry placement, the gates IN THE ORDER PRODUCTION RUNS THEM** (`runArmedPlacement` :1180, stop branch :965-990, WAVE B 2026-09-05): canonical side folded once at the row (:960, class 77) → `stopEntrySeamOn` (:94 — **OFF at the 2026-09-05 cutover by owner ruling: NO stop entry is placed at all, and the D4 boot line leads with it**) → retest window, FALLBACK ONLY — a reclaim skips it (`stopEntryNeedsRetestWindow` :1255) → `decideStopEntry` :1520 (**stop-side wrong-way guard**, tick-rounded trigger) → `placeOneStopEntry` :1567 → `PlaceStopEntry` :1552, and the **AddOn build floor is LAST**, inside that call (`tcp_trader.go:502`). ONE PREDICATE PER ORDER KIND, chosen by kind: `stopEntryMarketableWrongSide` :1418 is **at-or-through** (long `price >= trigger`, short `price <= trigger`) because a stop AT its trigger fires; `limitMarketableWrongSide` :1354 stays **strictly-through** because a limit AT its price rests. Three verdicts (`stopGuardVerdict` :1388, UNKNOWN = iota zero) mapped to three actions (`stopEntryAction` :1464, NO-OP = iota zero, and PLACEMENT IS REACHABLE ONLY BY NAMING `stopEntryPlace` :1469 — the `default:` arm no-ops): THROUGH → cancel `"accepted through (stop side): price %.2f %s trigger %.2f — never placed"` (:1281) · REST → place (:1312) · **UNKNOWN → no cancel, no placement**, WARN `"⚠️ armed %s stop-entry NOT adjudicated"` (:1293) + `countStopEntryRefusal` class `stop_entry:guard_unknown` (:1292) `[O]`. Build refusal: `ErrAddonBuildTooOld` → WARN + class `stop_entry:addon_build` (:1319). Refusal keys are 1-based and `:place`-namespaced (`armKey` :1576) so they cannot alias the arm-gate keys in the same map. `[R]` — until 2026-09-05 this branch called the LIMIT predicate with the trigger and all four of its answers were inverted: 21 already-through sell stops admitted on 2026-09-04 (armed_orders 38, 62-102), 0 valid stops ever placed, 0 fills on 22 post-E7 lifetime submissions.

**The side the wire carries** (class 77, 2026-09-05) `[R]`: `store.UpsertArm` canonicalizes `armed_orders.side` to **UPPERCASE** at the write (`store/armed_orders.go:181`, class 28) and the placement branch compared it to the lowercase literal `"long"`. Two silent consequences: a LONG stop entry was built at `entry − offset` (below the level a buy stop must sit above), and the wire carried `"LONG"` to a C# ternary reading `side == "long" ? Buy : SellShort` — a LONG entry, limit or stop, submitted as a live SELL. Never fired because no LONG row exists post-canonicalizer (21 stop_entry + 9 limit rows, all SHORT). Now folded ONCE at :960 and used for the trigger, the guard, the log and both wire calls (`decideStopEntry` :1520 folds again, defensively); the AddOn folds on arrival too (`VLTraderTCPClient.cs:975`).
<!-- /MAPCHECK -->
...
- Placement: `placeEntry` `tcp_trader.go:320` · `PlaceLimitEntry` :433 · `PlaceStopEntry` :489 → `tcp_server.go:1047 SendSignal`. `CancelOrder` :554 · `ModifyBracket` :562 · `MoveStopToBreakeven` via `SendMoveStop` :1078.
- **AddOn build floor on stop entries** (`tcp_trader.go:502`, WAVE B 2026-09-05): `PlaceStopEntry`'s FIRST check, ahead of the bound-account and SIM checks. `ntwire.FarSideProven(FarSideBuildID(), ntwire.MinAddonBuildStopSlot)` — refusal `"refusing stop-entry %s %s trigger=%.2f qty=%.0f [guard=far_side_build] — addon build predates the stop-slot fix (build_id=%s, need ≥ %s)…"` wrapping `ntwire.ErrAddonBuildTooOld` so the caller counts it apart from a transport failure. Constants: `MinAddonBuildStopSlot` `tcp_framing.go:241` (the gate) · `FarSideBuildE7` :230 (SUPERSEDED, gates nothing — it proved the AddOn PARSED a stop_entry frame, not that the trigger reached the stop slot) · `ExpectedAddonBuild` `order_snapshot.go:214` and C# `VL_BUILD_ID` `VLTraderTCPClient.cs:55` move in LOCKSTEP with it. `FarSideProven` compares BYTEWISE and suffixes are not zero-padded (`-f9` > `-f12`), so a new floor must advance the ISO DATE `[O]`.
...
- **Consecutive-loss breaker** — `trader/session_risk.go`. N consecutive losing closes in one CME session-day → no new entry on EITHER path until the roll; N resolves through `breakerHaltN` (owner knob, else `8` `[I]`; `BREAKER_HALT_N=0` disables), WARN at M=`5` `[I]`. NOT gated by the guardrails master. `[T]` On the retained tape (n=73 usable era closes) the longest run is SEVEN, ids 585-591 — N=8 never fires. An UNRESOLVED P&L **ends** a run rather than bridging it (`CountConsecutiveLossesSince`, `store/position_query.go`): bridging lengthens runs and so pushes toward a halt, which is the destructive branch (A24).
- **No-trade band on the ARM path** — `sessionEntryBlockedAt` (A28 seam) consulted once per cycle in `maybeManageArmedOrdersAt` before the scenario loop. `[R]` Previously enforced ONLY at `auto_trader_orders.go:281` on the decision path, which `plan_mode=strict` forbids from entering; `armed_executor.go` held zero references to it (class 95). An arm resting when the band opens is cancelled, not grandfathered.
...
**Boot line:** `"🛑 session risk: daily=… · breaker=…[I] warn=…[I] · no-trade-band=arm+decision · post-loss counter=on(…m) · flat@…=position+arms+pending"` `main.go` via `trader.SessionRiskBootLineForBoot`.
...
## Audit source execution-order boundary (2026-09-13, undeployed)

`provider/ninjatrader/ordered_execution.go` registers one exact account/symbol
execution owner. TCP `readLoop` applies order updates, fill-cache updates and
position-close receipts in receive order before advisory fanout. Internal
`json:"-"` handled markers prevent downstream duplicate accounting. Successful
AutoTrader construction installs the owner; a replacement cannot be removed by
the old owner's cleanup, and ordinary Stop retains exposure observation.
Callbacks must not synchronously await broker replies. Storage backpressure
delays further TCP reads; no durable inbound journal is claimed. Pre-owner
legacy queues are outside this ordering guarantee.

`ApplyNT8Exit` atomically records exact exit identity, actual quantity, residual
position and fill P&L. Terminal cancelled/rejected exits with actual fills are
reported by the AddOn; transient PartFilled is not charged twice. Cumulative
entry growth observed after a closed exact entry can record continuing exposure
from that same order; it does not authorize a new entry. Equal/older replay
cannot reopen it, and corrected final P&L becomes unresolved while open again.
This preserves receive-order accounting, not independent exchange timestamps or
retroactively corrected excursion/terminal analytics.
