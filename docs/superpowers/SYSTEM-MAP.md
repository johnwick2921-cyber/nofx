...
**Gate legs + refusals** (`armGateVerdictFor` :1316): invalid ArmSpec → err as-is (:1322) · direction not armable `"direction %q not armable"` (:1325) · plan bias `"against plan bias %q (plan_mode=direction)"` (:1335) · quality `"quality %s below min_scenario_quality %s"` (:1341) · R:R `"R:R %.2f below arm min %.2f (studio min_risk_reward_ratio)"` (:1353, one floor = `min_risk_reward_ratio`, default 3.0 `store/strategy.go:76`; ARM_MIN_RR env DELETED `[O]`) · min-SL `"stop %.2f too close (%.2f < %.2f = %.1f×ATR5m)"` (:1362) · HTF veto `"HTF veto: " + reason` (:1375). `oneLiveArmGuard` (:640, class-27 FIX 4): `"one_open_position: %s arm %s refused — position %d open …; no adds, no flips (owner ruling 2026-09-03)"` (:657-658). Marketable-side limits never placed: `"level accepted through — marketable, never placed"` (:988, `limitMarketableWrongSide` :1063 with the AUTHORED ENTRY) `[R]` 08-30 incident.

**Stop-entry placement, three gates in order** (`runArmedPlacement` :929-980, WAVE B 2026-09-05): `stopEntrySeamOn()` (:930) → retest window, FALLBACK ONLY — `reclaim` skips it (:936-941) → **stop-side wrong-way guard** `stopEntryGuardVerdict(r.Side, trigger, price)` :943 → `PlaceStopEntry` :982 (which refuses on the AddOn build floor). ONE PREDICATE PER ORDER KIND, chosen by kind: `stopEntryMarketableWrongSide` :1077 is **at-or-through** (long `price >= trigger`, short `price <= trigger`) because a stop AT its trigger fires; `limitMarketableWrongSide` :1063 stays **strictly-through** because a limit AT its price rests. Three verdicts (`stopGuardVerdict` :1030, UNKNOWN = iota zero): THROUGH → cancel `"accepted through (stop side): price %.2f %s trigger %.2f — never placed"` (:947) · REST → place · **UNKNOWN → no cancel, no placement**, WARN `"⚠️ armed %s stop-entry NOT adjudicated"` (:956) + `store.IncArmRefusal` class `stop_entry:guard_unknown` (:955) `[O]`. Build refusal: `errors.Is(perr, ntwire.ErrAddonBuildTooOld)` → WARN + class `stop_entry:addon_build` (:967-970). `[R]` — until 2026-09-05 this branch called the LIMIT predicate with the trigger and all four of its answers were inverted: 21 already-through sell stops admitted on 2026-09-04 (armed_orders 38, 62-102), 0 valid stops ever placed, lifetime fill rate 0/22.
...
- **The entry order's price slots** (`VLTraderTCPClient.cs:976-991`): `Account.CreateOrder` is positional — after `quantity` come (limitPrice, stopPrice, oco, name, gtd, customOrder). `limitArg = isLimit ? limitPx : 0`, `stopArg = isStopEntry ? stopPx : 0`. `[R]` Before 2026-09-05 both kinds shared one `orderPx` in the limitPrice slot with a literal 0 in stopPrice: every StopMarket entry reached NT8 as `Limit price=<trigger> Stop price=0` — accepted, listed in the book, never worked, never rejected (22 of 22 lifetime, 0 fills). The bracket SL at :1834 (`b.Qty, 0, b.Sl`) is the in-file control and is proven correct by a live fill (2026-09-03, 29355).
