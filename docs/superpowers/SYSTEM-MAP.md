...
| min-SL | `MinSLATRMultDefault=1.5` (was 1.0, ruling 2026-09-02), `MinSLTickClearance=2` | `"sl_too_tight: %.1f < %.1f×ATR (%.1f) — widen or skip"` | min_sl.go:40-68 · [R]/[O] |
...
| 5 | R:R at REAL execution price | `"entry_gate: R:R %.2f below floor %.2f at execution price %.4f (SL %.4f TP %.4f)"`; floor = `min_risk_reward_ratio` (default 3.0) | :237-257 `[O]` R1 single floor |
| 6 | min-SL ×ATR5m | `"entry_gate: stop %.2f too close (%.2f < %.2f = %.1f×ATR5m)"`; mult = `MinSLATRMult` 1.5 | :262-268 `[R]` |
...
- Breakeven: `maybeMoveStopToBreakeven` `auto_trader.go:148`, trigger `breakeven_trigger_points` default **50 pts** (:201, opt-in OFF; the "+40pt" figure exists only in tests and a boot comment — auto_trader_dayplan.go:68) `[T]`. WARN `"🎯 auto-breakeven: %s %s +%.1f pts in profit → stop moved to breakeven (entry %.2f)"` :189.
- Trailing: `defaultTrailingATRMult=2.0` × ATR(14,5m) after breakeven — auto_trader_trailing.go:22-30, rails R-A ratchet / R-B never-below-entry :94-112 `[T]`.
- **SUSPENDED by default:** `exitMechsSuspended()` returns true — `exit_mechs_suspend.go:33-41` ("suspended 2026-09-02 pending MFE data (wave 1A)"; Round-7: worst exit family of 15, 567k backtests `[R]`); env `EXIT_MECHS_SUSPENDED=0` re-enables.
...
*Map generated 2026-09-04 from code @ `492d2067` + settings registry + the 2026-08-30 knob census and 2026-09-02 belief census. Drift found and recorded: `BD_MIN_CLOSES` 1 (was 2), `MinSLATRMultDefault` 1.5 (was 1.0), breakeven default 50 (not 40), OR = first 5 min (IB = first 60 min), no wake-predicate cutover in production code.*
