...
**Stop-entry placement, the gates IN THE ORDER PRODUCTION RUNS THEM** (`runArmedPlacement` :898, stop branch :934-954, WAVE B 2026-09-05): canonical side folded once at the row (:924, class 77) → `stopEntrySeamOn` (:950 — **OFF at the 2026-09-05 cutover by owner ruling: NO stop entry is placed at all, and the D4 boot line leads with it**) → retest window, FALLBACK ONLY — a reclaim skips it (`stopEntryNeedsRetestWindow` :956) → `decideStopEntry` :967 (**stop-side wrong-way guard**, tick-rounded trigger) → `placeOneStopEntry` :968 → `PlaceStopEntry` :1275, and the **AddOn build floor is LAST**, inside that call (`tcp_trader.go:502`). ONE PREDICATE PER ORDER KIND, chosen by kind: `stopEntryMarketableWrongSide` :1082 is **at-or-through** (long `price >= trigger`, short `price <= trigger`) because a stop AT its trigger fires; `limitMarketableWrongSide` :1017 stays **strictly-through** because a limit AT its price rests. Three verdicts (`stopGuardVerdict` :1055, UNKNOWN = iota zero) mapped to three actions (`stopEntryAction` :1125, NO-OP = iota zero, and PLACEMENT IS REACHABLE ONLY BY NAMING `stopEntryPlace` :1136 — the `default:` arm no-ops): THROUGH → cancel `"accepted through (stop side): price %.2f %s trigger %.2f — never placed"` (:1209) · REST → place (:1226) · **UNKNOWN → no cancel, no placement**, WARN `"⚠️ armed %s stop-entry NOT adjudicated"` (:1219) + `countStopEntryRefusal` class `stop_entry:guard_unknown` (:1218) `[O]`. Build refusal: `ErrAddonBuildTooOld` → WARN + class `stop_entry:addon_build` (:1231). Refusal keys are 1-based and `:place`-namespaced (`armKey` :1241) so they cannot alias the arm-gate keys in the same map. `[R]` — until 2026-09-05 this branch called the LIMIT predicate with the trigger and all four of its answers were inverted: 21 already-through sell stops admitted on 2026-09-04 (armed_orders 38, 62-102), 0 valid stops ever placed, 0 fills on 22 post-E7 lifetime submissions.
