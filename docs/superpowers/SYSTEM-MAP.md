...
**Stop-entry placement, the gates IN THE ORDER PRODUCTION RUNS THEM** (`runArmedPlacement` :1156, stop branch :963-990, WAVE B 2026-09-05): canonical side folded once at the row (:958, class 77) → `stopEntrySeamOn` (:92 — **OFF at the 2026-09-05 cutover by owner ruling: NO stop entry is placed at all, and the D4 boot line leads with it**) → retest window, FALLBACK ONLY — a reclaim skips it (`stopEntryNeedsRetestWindow` :1228) → `decideStopEntry` :1489 (**stop-side wrong-way guard**, tick-rounded trigger) → `placeOneStopEntry` :1534 → `PlaceStopEntry` :1521, and the **AddOn build floor is LAST**, inside that call (`tcp_trader.go:502`). ONE PREDICATE PER ORDER KIND, chosen by kind: `stopEntryMarketableWrongSide` :1387 is **at-or-through** (long `price >= trigger`, short `price <= trigger`) because a stop AT its trigger fires; `limitMarketableWrongSide` :1323 stays **strictly-through** because a limit AT its price rests. Three verdicts (`stopGuardVerdict` :1357, UNKNOWN = iota zero) mapped to three actions (`stopEntryAction` :1433, NO-OP = iota zero, and PLACEMENT IS REACHABLE ONLY BY NAMING `stopEntryPlace` :1438 — the `default:` arm no-ops): THROUGH → cancel `"accepted through (stop side): price %.2f %s trigger %.2f — never placed"` (:1279) · REST → place (:1310) · **UNKNOWN → no cancel, no placement**, WARN `"⚠️ armed %s stop-entry NOT adjudicated"` (:1291) + `countStopEntryRefusal` class `stop_entry:guard_unknown` (:1290) `[O]`. Build refusal: `ErrAddonBuildTooOld` → WARN + class `stop_entry:addon_build` (:1317). Refusal keys are 1-based and `:place`-namespaced (`armKey` :1543) so they cannot alias the arm-gate keys in the same map. `[R]` — until 2026-09-05 this branch called the LIMIT predicate with the trigger and all four of its answers were inverted: 21 already-through sell stops admitted on 2026-09-04 (armed_orders 38, 62-102), 0 valid stops ever placed, 0 fills on 22 post-E7 lifetime submissions.

**The side the wire carries** (class 77, 2026-09-05) `[R]`: `store.UpsertArm` canonicalizes `armed_orders.side` to **UPPERCASE** at the write (`store/armed_orders.go:181`, class 28) and the placement branch compared it to the lowercase literal `"long"`. Two silent consequences: a LONG stop entry was built at `entry − offset` (below the level a buy stop must sit above), and the wire carried `"LONG"` to a C# ternary reading `side == "long" ? Buy : SellShort` — a LONG entry, limit or stop, submitted as a live SELL. Never fired because no LONG row exists post-canonicalizer (21 stop_entry + 9 limit rows, all SHORT). Now folded ONCE at :958 and used for the trigger, the guard, the log and both wire calls (`decideStopEntry` :1489 folds again, defensively); the AddOn folds on arrival too (`VLTraderTCPClient.cs:975`).
<!-- /MAPCHECK -->
